...
@main_bp.route('/clubs/new', methods=['GET', 'POST'])
def new_club():
    if request.method == 'POST':
        ...
        if not year_raw.isdigit():
            ...
            return redirect(url_for('main.new_club'))

        if Club.query.filter_by(name=name).first():
            flash('ชื่อสโมสรนี้มีอยู่แล้ว', 'danger')
            return redirect(url_for('main.new_club'))

        club = Club(name=name, stadium=stadium, year=int(year_raw), logo=logo)
...
@main_bp.route('/clubs/search', methods=['GET', 'POST'])
def search_clubs():
    if request.method == 'POST':
        club_name = request.form.get('club_name', '').strip()
        return redirect(url_for('main.search_clubs', club_name=club_name))
...
def update_club(club_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('main.club_info', club_id=club.id))
...
@main_bp.route('/players/search', methods=['POST'])
def search_players():
...
def new_player():
    ...
    if request.method == 'POST':
        ...
        if not club_id_raw.isdigit():
            ...
            return redirect(url_for('main.new_player'))
...
@main_bp.route('/players/<int:player_id>/update', methods=['GET', 'POST'])
def update_player(player_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(url_for('main.player_detail', player_id=player.id))
        except IntegrityError:
            db.session.rollback()
            flash('อัปเดตไม่สำเร็จ', 'danger')
            return redirect(url_for('main.update_player', player_id=player.id))
